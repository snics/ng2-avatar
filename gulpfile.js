...
const config = {
  libraryName: 'ng2-avatar',
  unscopedLibraryName: 'ng2-avatar',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('transpileES2015', () =>
  gulp.src(`${config.outputDir}/**/*.js`)
    .pipe(babel({
      presets: ['es2015']
    }))
...
gulp.task('build', ['clean'], (cb) => {
  runSequence('compile', 'test', 'npm-package', 'rollup-bundle', 'transpileES2015', cb);
});
